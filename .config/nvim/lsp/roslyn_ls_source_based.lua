...
--     fs.joinpath(uv.os_tmpdir(), 'roslyn_ls_source_based/logs'),
--     '--stdio',
--   },
...
local group = vim.api.nvim_create_augroup("lspconfig.roslyn_ls_source_based", { clear = true })

---@param client vim.lsp.Client
---@param target string
local function on_init_sln(client, target)
  vim.notify("Initializing: " .. target, vim.log.levels.TRACE, { title = "roslyn_ls_source_based" })
...
local function on_init_project(client, project_files)
  vim.notify("Initializing: projects", vim.log.levels.TRACE, { title = "roslyn_ls_source_based" })
  ---@diagnostic disable-next-line: param-type-mismatch
...
local function roslyn_handlers()
  return {
    ["workspace/projectInitializationComplete"] = function(_, _, ctx)
      vim.notify("Roslyn project initialization complete", vim.log.levels.INFO, { title = "roslyn_ls_source_based" })
    ...
    ["workspace/_roslyn_projectNeedsRestore"] = function(_, result, ctx)
      ...
      client:request("workspace/_roslyn_restore", result, function(err, response)
        if err then
          vim.notify(err.message, vim.log.levels.ERROR, { title = "roslyn_ls_source_based" })
        end
        if response then
          for _, v in ipairs(response) do
            vim.notify(v.message, vim.log.levels.INFO, { title = "roslyn_ls_source_based" })
          end
        end
      end)
...
local home = uv.os_getenv("HOME")

---@type vim.lsp.Config
return {
  name = "roslyn_ls_source_based",
  offset_encoding = "utf-8",
  cmd = {
    "dotnet",
    fs.joinpath(
      home,
      "src/vendor/roslyn/artifacts/bin/Microsoft.CodeAnalysis.LanguageServer/Debug/net9.0/Microsoft.CodeAnalysis.LanguageServer.dll"
    ),
    "--logLevel",
    "Debug",
    "--telemetryLevel",
    ...
    fs.joinpath(
      home,
      "src/vendor/roslyn/artifacts/bin/Microsoft.VisualStudio.LanguageServices.DevKit/Debug/net9.0/Microsoft.VisualStudio.LanguageServices.DevKit.dll"
    ),
  },
  filetypes = { "cs" },
  ...
  commands = {
    ["roslyn.client.completionComplexEdit"] = function(command, ctx)
      ...
      if uri and edit and edit.newText and edit.range then
        ...
        vim.notify(
          "roslyn_ls_source_based: completionComplexEdit args not understood: " .. vim.inspect(args),
          vim.log.levels.WARN
        )
      end
    end,
  ...
  on_attach = function(client, bufnr)
    ...
    vim.api.nvim_create_autocmd({ "BufWritePost", "InsertLeave" }, {
      ...
      desc = "roslyn_ls_source_based: refresh diagnostics",
    })
  end,

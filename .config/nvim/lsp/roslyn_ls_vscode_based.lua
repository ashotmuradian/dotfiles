...
local function on_init_sln(client, target)
  vim.notify("Initializing: " .. target, vim.log.levels.TRACE, { title = "roslyn_ls_vscode_based" })
...
local function on_init_project(client, project_files)
  vim.notify("Initializing: projects", vim.log.levels.TRACE, { title = "roslyn_ls_vscode_based" })
  ---@diagnostic disable-next-line: param-type-mismatch
...
local function roslyn_handlers()
  return {
    ["workspace/projectInitializationComplete"] = function(_, _, ctx)
      vim.notify("Roslyn project initialization complete", vim.log.levels.INFO, { title = "roslyn_ls_vscode_based" })
    ...
    ["workspace/_roslyn_projectNeedsRestore"] = function(_, result, ctx)
      ...
      client:request("workspace/_roslyn_restore", result, function(err, response)
        if err then
          vim.notify(err.message, vim.log.levels.ERROR, { title = "roslyn_ls_vscode_based" })
        end
        if response then
          for _, v in ipairs(response) do
            vim.notify(v.message, vim.log.levels.INFO, { title = "roslyn_ls_vscode_based" })
          end
        end
      end)
...
return {
  ...
  commands = {
    ["roslyn.client.completionComplexEdit"] = function(command, ctx)
      ...
      if uri and edit and edit.newText and edit.range then
        ...
        vim.notify(
          "roslyn_ls_vscode_based: completionComplexEdit args not understood: " .. vim.inspect(args),
          vim.log.levels.WARN
        )
      end
    end,
  ...
  on_attach = function(client, bufnr)
    ...
    vim.api.nvim_create_autocmd({ "BufWritePost", "InsertLeave" }, {
      ...
      desc = "roslyn_ls_vscode_based: refresh diagnostics",
    })
  end,
